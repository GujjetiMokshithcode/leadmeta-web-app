...
export type HeroWaveProps = {
  ...
  placeholder?: string;
  buttonText?: string;
  onPromptSubmit?: (value: string, targetCount: number, mode: 'ai' | 'manual') => void;
};
    ...
    >
      ...
      >
        ...
        >
          ...
          <form
            className="mt-6 sm:mt-8 flex flex-col items-center justify-center gap-4"
            onSubmit={(e) => {
              e.preventDefault();
              onPromptSubmit?.(prompt, targetCount, mode);
            }}
          >
              ...
              <div className="inline-flex p-1 bg-white/5 rounded-full border border-white/10 backdrop-blur-md">
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${
                    mode === 'ai' ? 'bg-white text-black shadow-sm' : 'text-white/60 hover:text-white'
                  }`}
                >
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${
                    mode === 'manual' ? 'bg-white text-black shadow-sm' : 'text-white/60 hover:text-white'
                  }`}
                >
            ...
            <div className="relative w-full sm:w-[720px]">
              <div className="relative rounded-2xl p-[2px] shadow-[0_1px_2px_0_rgba(0,0,0,0.06)] bg-gradient-to-br from-white/10 via-white/5 to-black/20">
                <textarea
                  ...
                  placeholder={mode === 'ai' ? animatedPlaceholder : "Enter a specific search query, e.g., site:linkedin.com/in/ 'CEO' '@gmail.com'"}
                  rows={5}
              ...
              <div className="absolute left-3 bottom-3 flex items-center gap-2 border border-white/10 px-3 py-1.5 rounded-lg bg-black/40 backdrop-blur-md z-10">
                 <span className="text-[10px] font-bold text-white/60 uppercase">Target</span>
                 <input
                   type="number"
                   value={targetCount}
              ...
              <button
                ...
                className="absolute right-3 bottom-3 inline-flex items-center justify-center w-10 h-10 rounded-xl bg-[#f0f2ff] text-black hover:bg-white transition-colors z-10"
              >
