...
function DashboardContent() {
  ...
  const initialMode = (searchParams.get('mode') as 'ai' | 'manual') || 'ai';
  ...
  useEffect(() => {
    if (initialQuery && results.length === 0 && !loading && !isSearching) {
      handleSearch(initialQuery, initialTarget, initialMode);
    }
  }, [initialQuery]);
  ...
  const handleNewSearch = () => {
    ...
    handleSearch(newQuery, targetCount, 'ai');
    setNewQuery('');
  };
        ...
        <div className="container mx-auto px-4 max-w-6xl h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <div className="h-8 w-8 rounded-lg bg-[#1f3dbc] flex items-center justify-center">
                <span className="text-sm font-bold text-white">L</span>
              </div>
              <span className="font-semibold">Leadmeta</span>
            </div>
          </div>
          
          <div className="flex items-center gap-4">
            <Badge variant="outline" className="bg-white/5">
              <Activity className={`h-3 w-3 mr-2 ${isSearching ? 'text-green-400 animate-pulse' : 'text-muted-foreground'}`} />
          ...
          <CardContent className="p-4">
            <div className="flex gap-3">
              ...
              <Button 
                ...
                className="bg-[#1f3dbc] hover:bg-[#1f3dbc]/90"
              >
  ...
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="flex items-center gap-3">
          <RefreshCw className="h-5 w-5 animate-spin text-[#1f3dbc]" />
